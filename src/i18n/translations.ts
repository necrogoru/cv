export const translations = {
  en: {
    firstName: "Jhon F.",
    lastName: "Berrio",
    current_role: "Frontend Tech Lead",
    profile: "Frontend Tech Lead specializing in Vue, Nuxt, frontend architecture, developer experience, and AI-assisted engineering workflows. I lead frontend teams, define technical direction, improve delivery systems, and partner with product teams to turn ambiguous needs into scalable software solutions.",
    articles: {
      at: "at",
      in: "in"
    },
    information: [
      { kind: "Phone", icon: "mobile_3", value: "+57 3146022349"},
      { kind: "Email", icon: "alternate_email", value: "jhoxsan@gmail.com"},
      { kind: "City", icon: "location_on", value: "Medellín, Antioquia"}
    ],
    skills: [
      { name: "Vue", level: 5 },
      { name: "Nuxt", level: 5 },
      { name: "TypeScript", level: 5 },
      { name: "JavaScript", level: 5 },
      { name: "Tailwind CSS", level: 5 },
      { name: "Vitest", level: 4 },
      { name: "Node.js", level: 3 },
      { name: "Rails", level: 3 },
      { name: "Astro", level: 3 },
      { name: "React", level: 2 }
    ],
    academy: [
      {
        college: "ITM",
        type: "University",
        title: "Telecommunications Engineer",
        time: "09-2018"
      },{
        college: "ITM",
        type: "Technologist",
        title: "Telecommunications Technologist",
        time: "2010 to 2013"
      }
    ],
    experience: [
      {
        company: "8020REI",
        appointment: "Frontend Tech Lead",
        functions: "Lead the frontend team and define technical direction for frontend and full-stack initiatives. Created a new Nuxt frontend and led the migration of product modules. Built a full-stack platform from scratch with Nuxt and FastAPI — database modeling, API architecture, and deployments on AWS (EC2, Lambda, Amplify) — and integrated third-party vendors for payments and direct mail. Defined the design system: reusable components, UX patterns, and implementation standards, with technical guardrails for AI coding agents to enforce consistency and reuse. Built AI-assisted development workflows (reusable skills, shared context, agent structures) applied across architecture, implementation, testing, and documentation. Partnered with product on solutions and trade-offs. Automated code quality, testing, semantic versioning, and deployments with GitHub Actions; implemented observability and analytics.",
        time: "May 2024 - Present",
        tags: ["Vue", "Nuxt", "TypeScript", "Claude", "Codex", "Kimi", "OpenCode", "Python", "FastAPI", "EC2", "Lambda", "S3", "Amplify", "PostgreSQL", "Cloudflare", "Vitest", "GA4", "Rollbar", "Heap", "Semantic Release", "GitHub Actions", "CI/CD", "Vite", "Design System", "Payments"]
      }, {
        company: "La Haus",
        appointment: "Senior Frontend Developer",
        functions: "Developed public views using Vanilla JavaScript and Ruby on Rails, with focus on performance, SEO, accessibility, and traceability. Designed, developed, and deployed web apps using NuxtJS and VueJS. Updated JavaScript dependencies and adjusted bundles with Webpack and Vite to improve site performance. Designed the technical foundation of the design system with Vue, Storybook, JavaScript and CSS linters, Semantic Release for automatic versioning, and GitHub workflows to automate testing, version generation, and NPM publishing. Integrated monitoring, observability, and traceability tools across frontend and backend systems, including Google Analytics, Segment, Heap, and Hotjar.",
        time: "April 2022 - April 2024",
        tags: ["Vue", "Nuxt", "JavaScript", "TypeScript", "Vitest", "Datadog", "GA4", "Storybook", "Semantic Release", "Segment", "Heap", "Hotjar", "Vite", "Webpack", "Ruby on Rails", "SEO", "NPM"]
      }, {
        company: "La Haus",
        appointment: "Mid Full-stack Developer",
        functions: "Developed the platform search engine and recommendation system using Vue, Algolia, InstantSearch, and Ruby on Rails. Migrated views from Ruby on Rails to NuxtJS. Developed and implemented a server-side rendered frontend with NuxtJS, including style and JavaScript linters, tests, repository configuration, Heroku setup with buildpacks, environment variables, plugins, review apps, staging and production environments, Rollbar and Datadog observability services, and traceability integrations.",
        time: "May 2019 - April 2022",
        tags: ["Vue", "Nuxt", "JavaScript", "Ruby on Rails", "Algolia", "Heroku", "Rollbar", "Datadog"]
      }, {
        company: "La Haus",
        appointment: "Junior Full-stack Developer",
        functions: "Developed administrative platform views using VueJS for the frontend and Ruby on Rails for the backend.",
        time: "November 2018 - May 2019",
        tags: ["Vue", "Nuxt", "JavaScript", "Ruby on Rails", "PostgreSQL"]
      }, {
        company: "TIGOUNE",
        appointment: "Operations Support",
        functions: "Designed, developed, and implemented the SME back-office platform and its databases using Ruby on Rails, JavaScript, HTML, SCSS, Materialize, VueJS, and PostgreSQL. Administered the platform, integrated SOAP web services for certified mass messaging, and developed real-time and non-real-time dashboards to monitor operational status and indicators.",
        time: "September 2017 - October 2018"
      }, {
        company: "EMTELCO",
        appointment: "On-site BPO Management Support Programming and Control Analyst",
        functions: "Analyzed indicators, payroll, and invoicing. Managed quotas, administered databases, and developed management platforms using Ruby on Rails, PHP, VueJS, MySQL, HTML5, CSS3, jQuery, Bootstrap, and Material Design. Advanced Excel user.",
        time: "March 2015 – September 2017"
      }, {
        company: "EMTELCO",
        appointment: "Programming and Control Analyst",
        functions: "Prepared reports and supported payroll and invoicing analysis. Advanced Excel user.",
        time: "January 2015 –  March 2015"
      }, {
        company: "EMTELCO",
        appointment: "Virtual Content Manager",
        functions: "Virtualized operational processes and information content.",
        time: "March 2014 – January 2015"
      }
    ],
    references: {
      personal: [
        {
          name: "Juliana Castrillón Guzmán",
          job: "Founder and CEO",
          company: "La Haus",
          phone: "+57 3117461913"
        }, {
          name: "Alexandra Lopera Márquez",
          job: "Founder and COO",
          company: "La Haus",
          phone: "+57 3108041084"
        }
      ]
    },
    interests: ["Coding", "Photography", "Motorcycles", "Camping", "Astronomy"]
  },
  es: {
    firstName: "Jhon F.",
    lastName: "Berrio",
    current_role: "Frontend Tech Lead",
    profile: "Frontend Tech Lead especializado en Vue, Nuxt, arquitectura frontend, experiencia de desarrollo y flujos de ingeniería asistidos por IA. Lidero equipos frontend, defino dirección técnica, mejoro sistemas de entrega y acompaño a producto para convertir necesidades ambiguas en soluciones de software escalables.",
    articles: {
      at: "en",
      in: "en"
    },
    information: [
      { kind: "Teléfono", icon: "smartphone", value: "+57 3146022349"},
      { kind: "Correo", icon: "email", value: "jhoxsan@gmail.com"},
      { kind: "Ciudad", icon: "map", value: "Medellín, Antioquia"}
    ],
    skills: [
      { name: "Vue", level: 5 },
      { name: "Nuxt", level: 5 },
      { name: "TypeScript", level: 5 },
      { name: "JavaScript", level: 5 },
      { name: "Tailwind CSS", level: 5 },
      { name: "Vitest", level: 4 },
      { name: "Node.js", level: 3 },
      { name: "Rails", level: 3 },
      { name: "Astro", level: 3 },
      { name: "React", level: 2 }
    ],
    academy: [
      {
        college: "Instituto Tecnológico Metropolitano",
        type: "Universitarios",
        title: "Ingeniero en Telecomunicaciones",
        time: "09-2018"
      },{
        college: "Instituto Tecnológico Metropolitano",
        type: "Universitarios",
        title: "Tecnólogo en Telecomunicaciones",
        time: "2010 hasta 2013"
      }
    ],
    experience: [
      {
        company: "8020REI",
        appointment: "Frontend Tech Lead",
        functions: "Lidero el equipo de frontend y defino la dirección técnica de las iniciativas frontend y full-stack. Creé un nuevo frontend en Nuxt y lideré la migración de los módulos de producto. Construí una plataforma full-stack desde cero con Nuxt y FastAPI — modelado de base de datos, arquitectura de API y despliegues en AWS (EC2, Lambda, Amplify) — e integré proveedores externos para pagos y correo directo. Definí el design system: componentes reutilizables, patrones de UX y estándares de implementación, con reglas técnicas para agentes de codificación con IA que garantizan consistencia y reutilización. Construí flujos de desarrollo asistidos por IA (skills reutilizables, contexto compartido, estructuras de agentes) aplicados en arquitectura, implementación, pruebas y documentación. Acompaño a producto en la definición de soluciones y trade-offs. Automaticé calidad de código, pruebas, versionado semántico y despliegues con GitHub Actions; implementé observabilidad y analítica.",
        time: "Mayo 2024 - Actualidad",
        tags: ["Vue", "Nuxt", "TypeScript", "Claude", "Codex", "Kimi", "OpenCode", "Python", "FastAPI", "EC2", "Lambda", "S3", "Amplify", "PostgreSQL", "Cloudflare", "Vitest", "GA4", "Rollbar", "Heap", "Semantic Release", "GitHub Actions", "CI/CD", "Vite", "Design System", "Payments"]
      }, {
        company: "La Haus",
        appointment: "Senior Frontend Developer",
        functions: "Desarrollé vistas públicas usando Vanilla JavaScript y Ruby on Rails, con foco en performance, SEO, accesibilidad y trazabilidad. Diseñé, desarrollé y llevé a producción web apps usando NuxtJS y VueJS. Actualicé dependencias de JavaScript y realicé ajustes de bundles con Webpack y Vite para mejorar el performance del sitio. Diseñé la base técnica del design system con Vue, Storybook, linters de JavaScript y CSS, Semantic Release para versionado automático y workflows de GitHub para automatizar pruebas, generación de versiones y publicación en NPM. Integré herramientas de monitoreo, observabilidad y trazabilidad para frontend y backend, incluyendo Google Analytics, Segment, Heap y Hotjar.",
        time: "Abril 2022 - Abril 2024",
        tags: ["Vue", "Nuxt", "JavaScript", "TypeScript", "Vitest", "Datadog", "GA4", "Storybook", "Semantic Release", "Segment", "Heap", "Hotjar", "Vite", "Webpack", "Ruby on Rails", "SEO", "NPM"]
      }, {
        company: "La Haus",
        appointment: "Mid Full-stack Developer",
        functions: "Desarrollé el buscador y el sistema recomendador de la plataforma usando Vue, Algolia, InstantSearch y Ruby on Rails. Migré vistas desde Ruby on Rails hacia NuxtJS. Desarrollé e implementé un frontend con renderizado del lado del servidor en NuxtJS, incluyendo linters de estilos y JavaScript, pruebas, configuración de repositorio, configuración de Heroku con buildpacks, variables de ambiente, plugins, review apps, ambientes de staging y producción, servicios de observabilidad con Rollbar y Datadog, y servicios de trazabilidad.",
        time: "Mayo 2019 - Abril 2022",
        tags: ["Vue", "Nuxt", "JavaScript", "Ruby on Rails", "Algolia", "Heroku", "Rollbar", "Datadog"]
      }, {
        company: "La Haus",
        appointment: "Junior Full-stack Developer",
        functions: "Desarrollé vistas administrativas de la plataforma usando VueJS para el frontend y Ruby on Rails para el backend.",
        time: "Noviembre 2018 - Mayo 2019",
        tags: ["Vue", "Nuxt", "JavaScript", "Ruby on Rails", "PostgreSQL"]
      }, {
        company: "TIGOUNE",
        appointment: "Soporte Operación",
        functions: "Diseñar, desarrollar e implementar la plataforma del área backoffice pymes y sus bases de datos bajo Ruby on Rails, Javascript, HTML, SCSS, Materialize, Vue JS, PostgreSQL. Administrar plataforma. Crear consumos de Web Services, bajo SOAP, de mensajería certificada para envíos masivos. Desarrollar real time y non-real time Dashboards para visualizar estados de la operación e indicadores.",
        time: "Septiembre 2017 - Octubre 2018"
      }, {
        company: "EMTELCO",
        appointment: "Analista de Programación y Control de Apoyo a Gerencia BPO Presencial",
        functions: "Analista de Indicadores, nómina y factura. Controlador de cupos, administrador de bases de datos y desarrollador de plataformas de la gerencia en Ruby on Rails, PHP, Vue JS, MySQL, HTML5, CSS3, JQuery, Bootstrap, Material Design. Manejo de Excel Avanzado.",
        time: "Marzo 2015 – Septiembre 2017"
      }, {
        company: "EMTELCO",
        appointment: "Analista de Programación y Control",
        functions: "Analista de reportes, nómina y factura. Manejo de Excel avanzado.",
        time: "Enero 2015 –  Marzo 2015"
      }, {
        company: "EMTELCO",
        appointment: "Gestor de contenidos virtuales",
        functions: "Virtualizar procesos y contenidos de información.",
        time: "Marzo 2014 – Enero 2015"
      }
    ],
    references: {
      personal: [
        {
          name: "Juliana Castrillón Guzmán",
          job: "Founder and CEO",
          company: "La Haus",
          phone: "+57 3117461913"
        }, {
          name: "Alexandra Lopera Márquez",
          job: "Founder and COO",
          company: "La Haus",
          phone: "+57 3108041084"
        }
      ]
    },
    interests: ["Coding", "Fotografía", "Motos", "Camping", "Astronomía"]
  }
} as const;

export type TranslationKey = keyof typeof translations.en;
